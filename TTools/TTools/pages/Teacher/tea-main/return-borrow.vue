<template>
	<view>
		<uni-section  title="归还审批" subTitleFontSize="16px" :subTitle="'未审批数量:'+borrow_list.length" titleFontSize="20px" type="line" >
				
					<uni-card v-for="(item, index) in borrow_list" :title="item.toolName" :sub-title="item.startTime+' 到 '+item.returnTime" :extra="'数量:'+item.borrowCount" :thumbnail="avatar" @click="onClick">
						...
						<uni-row class="demo-uni-row" :width="nvueWidth">
								...
								<uni-col :span="8">
									<view class="demo-uni-col uni-dark">
										<button  style="background-color:palegreen;" class="uni-button" size="mini" type="default"  @click="approve(item.requestId,1)">确认归还</button>
									</view>
								</uni-col>
